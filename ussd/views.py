...
def ussd_api(request):
    if request.method == 'POST':
        ...
        # Only process if the user ID is valid
        if user_id == 'GODEY100':
            ...
            # Process further based on session level
            level = session.level
            if level == 'start':
                ...
                if user_data == '1':
                    ...
                    session.delete()
                    message = "You have cancelled the process."
                    return JsonResponse(send_response(message, False))
                else:
                    session.delete()
                    message = "Invalid input. Session ended."
                    return JsonResponse(send_response(message, False))
                ...
                if response.status_code == 200:
                    message = f"You are about to pay GH¢{amount}. Please approve the payment on your phone."
                    return JsonResponse(send_response(message, True))
                else:
                    session.delete()
